...
namespace GV {
	static std::string stringFromShaderType(GLenum type) {
		...
		GV_GFX_ERROR("Unknown Shadertype: {0}", type);
		return "";
	}
	static GLenum shaderTypeToGl(ShaderType type) {
		...
		GV_GFX_ERROR("Unknown Shadertype: {0}", (uint32_t)type);
		return GL_NONE;
	}
	...
	void OpenGLShader::compile(const ShaderSource& sources){
		...
		for (uint32_t i = 0; i < sources.Size(); i++) {
			...
			if (isCompiled == GL_FALSE) {
				...
				GV_ASSERT_MSG(false, "GLSL {0}-Shader compilation error of {1}:\n{2}", stringFromShaderType(type), m_name, infoLog.data());
				break;
			}
		...
		if (isLinked == GL_FALSE) {
			...
			GV_ASSERT_MSG(false, "Could not link shader program:{0}\n", infoLog.data());
			return;
		}
